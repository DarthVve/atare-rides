...
async function DataBaseInitialization() {
  await knex.schema.hasTable('passengers').then(async function (exists: unknown) {
    if (!exists) {
      await knex.schema
        .createTable('passengers', (table: { increments: (arg0: string) => void; integer: (arg0: string) => number; string: (arg0: string) => void; boolean: (arg0: string) => boolean; }) => {
          table.increments('id');
        ...
        .createTable('drivers', (table: { increments: (arg0: string) => void; integer: (arg0: string) => number; string: (arg0: string) => void; boolean: (arg0: string) => boolean; }) => {
          table.increments('id');
          table.string('firstname');
          table.string('lastname');
          table.string('email');
